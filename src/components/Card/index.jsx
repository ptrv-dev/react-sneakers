...
export const Card = ({
	...
	const onClickFavorite = () => {
		if (favorite.includes(id))
			return setFavorite((prev) => prev.filter((el) => el !== id));
		setFavorite((prev) => [...prev, id]);
	};
	const onClickPlus = () => {
		if (basket.includes(id))
			return setBasket((prev) => prev.filter((el) => el !== id));
		...
		<div className={style.card}>
			<button
				...
				className={
					style.card__button +
					" " +
					style.cardButtonFavorite +
					(favorite.includes(id) ? " " + style.active : "")
				}
			>
				<svg
					width="15"
			...
			<div className={style.card__row}>
				...
				{basket.includes(id) ? (
					...
					<button onClick={onClickPlus} className={style.card__button + " " + style.cardButtonPlus}>
						<svg
							width="12"
							height="12"
							viewBox="0 0 12 12"
